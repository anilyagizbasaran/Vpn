...
/// Response of `POST /peers` and `GET /peers/:id/config`.
///
/// [privateKey] is non-null only on creation. Afterwards [conf] carries the
/// `<PRIVATE_KEY>` placeholder that the app fills from secure storage.
class PeerConfig {
  const PeerConfig({
    required this.peer,
    required this.conf,
  ...
  /// Substitutes the locally stored private key into the served config.
