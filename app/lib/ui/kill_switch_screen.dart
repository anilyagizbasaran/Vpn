...
class KillSwitchScreen extends StatefulWidget {
  const KillSwitchScreen({super.key, SystemSettings? settings})
    : _settings = settings;
...
class _KillSwitchScreenState extends State<KillSwitchScreen> {
  late final SystemSettings _settings = widget._settings ?? SystemSettings();
  String? _error;

  Future<void> _open() async {
    final opened = await _settings.openVpnSettings();
    if (!mounted) return;
    setState(() {
      _error = opened
          ? null
